...
import cn.javaboy.admin.common.domain.sys.dto.OrderItemDTO;
import cn.javaboy.admin.common.domain.sys.dto.PageParamDTO;
...
public class JBPageUtil {
    ...
    public static <T> Page<T> convert2QueryPage(PageParamDTO baseDTO) {
        ...
        if (orders != null && !orders.isEmpty()) {
            List<com.baomidou.mybatisplus.core.metadata.OrderItem> orderItemList = orders.stream().map(JBPageUtil::convertOrderItem).collect(Collectors.toList());
    ...
    public static <T, E> PageResultDTO<T> convert2PageResult(IPage page, List<E> sourceList, Class<T> targetClazz) {
        ...
        List<T> records = JBBeanUtil.copyList(sourceList, targetClazz);
