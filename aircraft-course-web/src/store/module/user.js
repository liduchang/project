...
import { RESOURCE_TYPE_ENUM } from '@/constants/resource';
...
export default {
  ...
  mutations: {
    ...
    // 记住用户信息
    setRememberUserInfo (state, rememberUserInfo) {
      state.rememberUserInfo = rememberUserInfo;
      localSave('rememberUserInfo', JSON.stringify(rememberUserInfo));
    },
    setUserPrivilege (state, privilegeList) {
      ...
      for (const privilege of privilegeList) {
        // 是菜单权限
        if (privilege.type === RESOURCE_TYPE_ENUM.MENU.value) {
        ...
        // 如果是功能点
        if (privilege.type === RESOURCE_TYPE_ENUM.POINTS.value) {
  ...
  getters: {
    user: state => state.userLoginInfo,
    rememberUser: state => localRead('rememberUserInfo'),
    // 用户功能点权限
