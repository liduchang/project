    ...
    <Row :gutter="10">
      ...
      <Col :lg="5" :md="8">
        <Card class="warp-card" dis-hover>
          ...
          <Row>
            <Input
              @on-change="getListUserByDepartmentName"
              placeholder="输入组织机构名称"
      ...
      <!--Col 左侧公司组织列表 end-->
      <!--UserTable 表单列表模块 start-->
      <UserTable :departments="departmentGroup" :selectDepartment="selectDepartment"></UserTable>
      <!--UserTable 表单列表模块 end-->
      <!--Modal 添加部门弹窗 start-->
      <Modal
          ...
          <FormItem class="selectClass" label="负责人">
            ...
            <div class="departmentWrap">
              <DepartmentUserTree
                ...
                @on-select="selectDepartmentUser"
...
export default {
  ...
  mounted () {
    ...
    this.getListUserByDepartmentName();
    let self = this;
  ...
  methods: {
    // 初始化加载数据
    async getListUserByDepartmentName () {
      this.$Spin.show();
      let result = await departmentApi.getListUserByDepartmentName(
        this.departmentName
      );
      this.$Spin.hide();
                    ...
                    h(
                      'Button',
                      {
                        ...
                        on: {
                          click: event => {
                            console.log('11');
                            this.loadUserTable(event, root, node, data);
                          }
                        }
                      },
                      newName
    ...
    // 选中部门 更新员工table
    loadUserTable (event, root, node, data) {
    ...
    addOrUpdataDepartment (data, update) {
      ...
      if (update) {
        ...
        this.selectUserId = data.managerId;
      } else {
    ...
    showTree () {
      if (this.isShowTree) {
        ...
        $('.departmentWrap').show();
        this.$refs.departmentUserTree.resetSearch();
      }
    },
    // 选中树形一项
    selectDepartmentUser (emp) {
    ...
    changeVisibleOperationModal (showStatus) {
      if (!showStatus) {
        this.isShowTree = false;
        $('.departmentWrap').hide();
        this.$refs.departmentUserTree.generateTreeData();
      }
    },
    // 部门上移或者下移
