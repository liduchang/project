...
export default {
  ...
  methods: {
    ...
    async buildPrivilegeTree () {
      this.$Spin.show();
      let getMenuListResult = await resourceApi.getMenuList();
    ...
    convert2Menu (router, parent) {
      return {
        type: RESOURCE_TYPE_ENUM.MENU.value,
    ...
    async addBatchSavePoints () {
      this.$Spin.show();
      let result = await resourceApi.addBatchSavePoints(
    ...
    async addBatchSaveMenu () {
      this.$Spin.show();
      let result = await resourceApi.addBatchSaveMenu(this.menuList);
      this.$Message.success('批量保存成功');
      this.$Spin.hide();
      // 重新获取数据
      this.initRouters();
    ...
    async getPrivilegeList (menuKey, frontPrivilegeData) {
      this.$Spin.show();
      this.formData.show = false;
      let result = await resourceApi.queryPrivilegeFunctionList(menuKey);
      this.$Spin.hide();
      // 处理服务端存储的功能点
