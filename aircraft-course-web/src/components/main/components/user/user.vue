...
export default {
  ...
  methods: {
    logout () {
      let re = this.rememberUser;
      this.$Spin.show();
      let token = cookie.getToken();
      // localStorage.clear();
      // 只清除登录的用户信息，不清除记住我的用户信息
      this.$store.commit('setUserLoginInfo', null);
      cookie.clearToken();
    ...
    async savePassword () {
      this.$Spin.show();
      // 密码加密
      this.formValidate.oldPwd = sha1(this.formValidate.oldPwd);
      this.formValidate.pwd = sha1(this.formValidate.pwd);
      this.formValidate.pwdAgain = sha1(this.formValidate.pwdAgain);

      let result = await userApi.updatePwd(this.formValidate);
